...
int main()
{
	...
	while (c != 0)
	{
		...
		case 2:
			...
			frontBackSplitLinkedList(&ll, &resultFrontList, &resultBackList); // 이 함수를 작성해야 합니다
...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	int halfIndex=0;
	if((ll->size % 2) == 1){
		halfIndex = ll->size/2 +1;
	}
	else{
		halfIndex = ll->size/2;
	}
	// size가 5면 half는 2+1 =3 
	...
	for(int i = 0; i < ll->size; i++){
		...
		if(i < halfIndex){
			insertNode(resultFrontList, resultFrontList->size, curItem);
		}
		else{
			insertNode(resultBackList, resultBackList->size, curItem);
		}
	}
}
